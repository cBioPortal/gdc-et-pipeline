...
import org.cbio.gdcpipeline.util.ClinicalLineAggregator;
import org.cbio.gdcpipeline.util.ClinicalSampleFileHeaders;
 ...
 */
public class ClinicalSampleDataWriter implements ResourceAwareItemWriterItemStream<CBioClinicalDataModel> {
    ...
    private String CLINICAL_DATA_SAMPLE_FILE_NAME;

    @Value("${clinical.metadata.sample.file.name}")
    private String CLINICAL_METADATA_FILE_NAME;

    FlatFileItemWriter<SampleFileModel> dataWriter = new FlatFileItemWriter<>();

    @Autowired
    ClinicalSampleFileHeaders sampleFileHeaders;
    ...
    private FlatFileHeaderCallback clinicalDataHeader() {
        return new FlatFileHeaderCallback() {
            ...
            public void writeHeader(Writer writer) throws IOException {
                ...
                StringBuilder sb = new StringBuilder();
                sb.append(modifyRow(sampleFileHeaders.getRow1()));
                sb.append(System.lineSeparator());
                sb.append(modifyRow(sampleFileHeaders.getRow2()));
                sb.append(System.lineSeparator());
                sb.append(modifyRow(sampleFileHeaders.getRow3()));
                sb.append(System.lineSeparator());
                sb.append(modifyRow(sampleFileHeaders.getRow4()));
                sb.append(System.lineSeparator());
                sb.append(modifyRow(sampleFileHeaders.getRow5()));
                //sb.append(System.lineSeparator());
    ...
    public FieldExtractor<SampleFileModel> createFieldExtractor() {
        BeanWrapperFieldExtractor<SampleFileModel> ext = new BeanWrapperFieldExtractor<>();

        String[] fields = sampleFileHeaders.getRow5().clone();
    ...
    public void open(ExecutionContext executionContext) throws ItemStreamException {
        ...
        ClinicalLineAggregator<SampleFileModel> lineAggregator = new ClinicalLineAggregator<>();
        lineAggregator.setDelimiter("\t");
        FieldExtractor<SampleFileModel> fe = createFieldExtractor();
        ...
        if (outputDir == null || outputDir.isEmpty()) {
            ...
            dir.mkdir();
            dataOutputFile = outputDir + File.separator + CLINICAL_DATA_SAMPLE_FILE_NAME;
    ...
    public void write(List<? extends CBioClinicalDataModel> cBioDataList) throws Exception {
        ...
        List<SampleFileModel> sampleDataList = new ArrayList<>();

        for (CBioClinicalDataModel cBioData : cBioDataList) {
            sampleDataList.add(cBioData.getSampleFileModel());
        }
        try {
            dataWriter.write(sampleDataList);
        } catch (Exception e) {
